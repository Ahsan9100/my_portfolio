/* =============================================
   MUHAMMAD AHSAN PORTFOLIO — script.js
...
(function () {
  ...
  if (!el) return;
  const words = ["Developer", "App Builder", "UI Creator", "Problem Solver", "Mobile Dev"];
  let i = 0;
...
(function () {
  if (typeof Lenis !== 'undefined') {
    ...
    lenis.on('scroll', (e) => {
      // Don't interrupt a programmatic scroll
      if (isScrollingToSection) return;

      clearTimeout(snapTimeout);
      
      snapTimeout = setTimeout(() => {
        // If the user's scroll velocity is very low, let's look for the best section to snap to
        if (Math.abs(e.velocity) < 0.5) {
          ...
          sections.forEach(sn => {
            const sTop = sn.offsetTop;
            const sHeight = sn.offsetHeight;
            const sBottom = sTop + sHeight;

            // Calculate how much of this section is in the viewport
            const overlapTop = Math.max(vTop, sTop);
            const overlapBottom = Math.min(vBottom, sBottom);
            const overlapHeight = Math.max(0, overlapBottom - overlapTop);
            const coverage = overlapHeight / vHeight;
          ...
          // If a section covers > 50% or is the best candidate and we aren't already there
          if (winner && maxCoverage > 0.05 && Math.abs(window.scrollY - winner.offsetTop) > 10) {
            isScrollingToSection = true;
            lenis.scrollTo(winner, {
              duration: 1.2,
              easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
              onComplete: () => {
